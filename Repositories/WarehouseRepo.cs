...
public class WarehouseRepo : IWarehouseRepo
{
    ...
    public async Task<int> FulfilledOrderId(InsertProductRequest insertProductReq)
    {
        ...
        cmd.CommandText = "SELECT IdOrder FROM \"Order\" WHERE IdProduct = @IdProduct AND Amount = @Amount AND FulfilledAt IS NOT NULL";
        cmd.Parameters.AddWithValue("@IdProduct", insertProductReq.IdProduct);
        cmd.Parameters.AddWithValue("@Amount", insertProductReq.Amount);
    ...
    public async Task<int> InsertProductInWarehouse(InsertProductRequest insertProductReq, int idOrder, int productPrice)
    {
        using var con = new SqlConnection(_configuration["ConnectionStrings:DefaultConnection"]);
        await con.OpenAsync();
